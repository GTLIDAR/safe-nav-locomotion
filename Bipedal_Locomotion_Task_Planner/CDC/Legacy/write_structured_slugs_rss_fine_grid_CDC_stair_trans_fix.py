...
def write_to_slugs_part_dist(infile,gw,init,initmovetarget,invisibilityset,PUDO_targets,visset_target = [],targets = [],vel=1,visdist = 5,allowed_states = [],
    ...
    #### Walking North ####
    stri = "((orientation=0 & turn=2) | (orientation=15 & turn=3) | (orientation=1 & turn=1))  & s>{} & forward & (stepL=0 | stepL=1) -> s' + {} = s\n".format(3*gw.ncols-1, 3*gw.ncols)

    stri += "((orientation=0 & turn=2) | (orientation=15 & turn=3) | (orientation=1 & turn=1)) & "
    stri += "s<{} & ".format(3*gw.ncols)
    stri += "forward & (stepL=0 | stepL=1) -> s' = s + {}\n".format((gw.nrows-3)*(gw.ncols))

    #### Walking East ####
    stri += "\n((orientation=3 & turn=3) | (orientation=4 & turn=2) | (orientation=5 & turn=1)) & "
    ...
    stri += "((orientation=3 & turn=3) | (orientation=4 & turn=2) | (orientation=5 & turn=1)) & ("
    for row in range(gw.nrows):
        stri += "s = {} \\/ s = {} \\/ s = {} \\/ ".format((row+1)*gw.ncols-3, (row+1)*gw.ncols-2, (row+1)*gw.ncols-1)
    stri = stri[:-4]
    stri += ") & forward & (stepL=0 | stepL=1) -> s' + {} = s\n".format(gw.ncols-3)
    ...
    #### Walking South ####
    stri += "\n((orientation=7 & turn=3) | (orientation=8 & turn=2) | (orientation=9 & turn=1)) & s<{} & forward & (stepL=0 | stepL=1) -> s' = s + {}\n".format((gw.nrows-3)*gw.ncols, 3*gw.ncols)

    stri += "((orientation=7 & turn=3) | (orientation=8 & turn=2) | (orientation=9 & turn=1)) & "
    stri += "s>{} &".format((gw.nrows-3)*gw.ncols-1)
    stri += "forward & (stepL=0 | stepL=1) -> s' + {} = s\n".format(gw.ncols*(gw.nrows-3))

    #### Walking West ####
    stri += "\n((orientation=11 & turn=3) | (orientation=12 & turn=2) | (orientation=13 & turn=1)) & "
    for row in range(gw.nrows):
        stri += "s != {} & s != {} & s != {} & ".format(row*gw.ncols+2 , row*gw.ncols+1 , row*gw.ncols)
    stri += "forward & (stepL=0 | stepL=1) -> s' + 3 = s\n"

    stri += "((orientation=11 & turn=3) | (orientation=12 & turn=2) | (orientation=13 & turn=1)) & ("
    for row in range(gw.nrows):
        stri += "s = {} \\/ s = {} \\/ s = {} \\/ ".format(row*gw.ncols+2 , row*gw.ncols+1 , row*gw.ncols)
    stri = stri[:-4]
    stri += ") & forward & (stepL=0 | stepL=1) -> s' = s + {}\n".format(gw.ncols-3)
    ...
    file.write(" (directionrequest' != 4 & directionrequest' != 2) -> !stair_n'\n")
    ...
    file.write("requestPending1 != 5 & stair_n-> stair_n' \n")
    file.write("requestPending1 != 5 & !stair_n-> !stair_n' \n")
