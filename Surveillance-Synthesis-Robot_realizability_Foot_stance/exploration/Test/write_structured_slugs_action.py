...
def write_to_slugs_part_dist(infile,gw,init,initmovetarget,invisibilityset,visset_target,targets,vel=1,visdist = 5,allowed_states = [],
    ...
    stri += "\n"
    file.write(stri)
    stri = "(s = 92) & (s' = 92) -> ! requestPending1'\n"
    stri += "!((s = 92) & (s' = 92)) ->(requestPending1' <-> (requestPending1 | deliveryrequest))\n\n"
    stri += "(s = 100) & (s' = 100) -> !requestPending2'\n"
    stri += "!((s = 100) & (s' = 100)) -> (requestPending2' <-> ((s = 92 & requestPending1) | requestPending2))\n\n"
    file.write(stri)
