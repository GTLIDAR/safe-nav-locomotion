...
def write_to_slugs_part_dist(infile,gw,init,initmovetarget,invisibilityset,visset_target,targets,vel=1,visdist = 5,allowed_states = [],
    ...
    stri += "\n"
    file.write(stri)
    stri = "(s = 189) & (s' = 189) -> ! requestPending1'\n"
    stri += "!((s = 189) & (s' = 189)) ->(requestPending1' <-> (requestPending1 | deliveryrequest))\n\n"
    stri += "(s = 53) & (s' = 53) -> !requestPending2'\n"
    stri += "!((s = 53) & (s' = 53)) -> (requestPending2' <-> ((s = 189 & requestPending1) | requestPending2))\n\n"
    file.write(stri)
