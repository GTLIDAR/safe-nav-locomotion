...
def write_to_slugs_part_dist(infile,gw,init,initmovetarget,invisibilityset,visset_target,targets,vel=1,visdist = 5,allowed_states = [],
    ...
    #     stri += "(orientation=3 | orientation=4 | orientation=2) & s > {} & s<{} & forward & stepL=0 -> s'=s+1\n".format(row*gw.ncols-1 , (row+1)*gw.ncols)

    # stri = "(orientation=6 | orientation=7 | orientation=5) & s<{} & forward & stepL=0 -> s = s + {}\n".format((gw.nrows-1)*gw.ncols, gw.ncols)
    ...
    stri = "\n(orientation=0 | orientation=11 | orientation=1)  & s>{} & forward & stepL=0 -> s' + {} = s\n".format(gw.ncols-1, gw.ncols)
    stri += "(orientation=0 | orientation=11 | orientation=1) & s>{} & forward & stepL=1 -> s' + {} = s\n".format(2*gw.ncols-1, 2*gw.ncols)
    ...
    file.write(stri)
    # stri = "(s = 1162) & (s' = 1162) -> ! requestPending1'\n"
    # stri += "!((s = 1162) & (s' = 1162)) ->(requestPending1' <-> (requestPending1 | deliveryrequest))\n\n"
    # stri += "(s = 818) & (s' = 818) -> !requestPending2'\n"
    # stri += "!((s = 818) & (s' = 818)) -> (requestPending2' <-> ((s = 1162 & requestPending1) | requestPending2))\n\n"
    file.write(stri)
    ...
    # file.write("(!forward' & stanceFoot' =2) \\/ (forward' \\/ stanceFoot' !=2\n")
    ...
    # file.write("!forward -> !turnLeft' /\\ turnRight'\n")
    # file.write('\n')
