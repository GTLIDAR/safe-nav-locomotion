...
AthenaCassieCommandSender::AthenaCassieCommandSender(int num_joints)
  ...
  this->DeclareAbstractOutputPort(
      "lcmt_cassie_command", &AthenaCassieCommandSender::CalcOutput);
}

using InPort = systems::InputPort<double>;
const InPort& AthenaCassieCommandSender::get_position_input_port() const {
  return LeafSystem<double>::get_input_port(0);
}
const InPort& AthenaCassieCommandSender::get_torque_input_port() const {
  return LeafSystem<double>::get_input_port(1);
}
const systems::OutputPort<double>& AthenaCassieCommandSender::get_output_port() const {
  return LeafSystem<double>::get_output_port(0);
}

void AthenaCassieCommandSender::CalcOutput(
