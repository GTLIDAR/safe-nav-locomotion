...
AthenaCassieStatusReceiver::AthenaCassieStatusReceiver(int num_joints)
    : num_joints_(num_joints) {
  this->DeclareAbstractInputPort(
      "lcmt_cassie_status", Value<lcmt_cassie_status>{});
  this->DeclareVectorOutputPort(
      "position_commanded", BasicVector<double>(num_joints_),
      &AthenaCassieStatusReceiver::CalcLcmOutput<
...
const systems::InputPort<double>& AthenaCassieStatusReceiver::get_input_port() const {
  return LeafSystem<double>::get_input_port(0);
}
using OutPort = systems::OutputPort<double>;
const OutPort& AthenaCassieStatusReceiver::get_position_commanded_output_port() const {
  return LeafSystem<double>::get_output_port(0);
}
const OutPort& AthenaCassieStatusReceiver::get_position_measured_output_port() const {
  return LeafSystem<double>::get_output_port(1);
}
const OutPort& AthenaCassieStatusReceiver::get_velocity_estimated_output_port() const {
  return LeafSystem<double>::get_output_port(2);
}
const OutPort& AthenaCassieStatusReceiver::get_torque_commanded_output_port() const {
  return LeafSystem<double>::get_output_port(3);
}
const OutPort& AthenaCassieStatusReceiver::get_torque_measured_output_port() const {
  return LeafSystem<double>::get_output_port(4);
}
const OutPort& AthenaCassieStatusReceiver::get_torque_external_output_port() const {
  return LeafSystem<double>::get_output_port(5);
}

template <std::vector<double> drake::lcmt_cassie_status::* field_ptr>
void AthenaCassieStatusReceiver::CalcLcmOutput(
...
void AthenaCassieStatusReceiver::SetInitialPosition(Eigen::VectorXd init_state){
