...
AthenaCassieCommandReceiver::AthenaCassieCommandReceiver(int num_joints)
    : num_joints_(num_joints) {
  DRAKE_THROW_UNLESS(num_joints > 0);

  message_input_ = &DeclareAbstractInputPort(
      "lcmt_cassie_command", Value<lcmt_cassie_command>());
  ...
  position_measured_or_param_ = &DeclareCacheEntry(
      ...
      &AthenaCassieCommandReceiver::CalcPositionMeasuredOrParam,
  ...
  defaulted_command_ = &DeclareCacheEntry(
      "defaulted_command", &AthenaCassieCommandReceiver::CalcDefaultedCommand,
  ...
  commanded_position_output_ = &DeclareVectorOutputPort(
      ...
      &AthenaCassieCommandReceiver::CalcPositionOutput,
      {defaulted_command_->ticket()});

  commanded_torque_output_ = &DeclareVectorOutputPort(
      "torque", BasicVector<double>(num_joints),
      &AthenaCassieCommandReceiver::CalcTorqueOutput,
      {defaulted_command_->ticket()});
}

void AthenaCassieCommandReceiver::set_initial_position(
...
void AthenaCassieCommandReceiver::CalcPositionMeasuredOrParam(
...
void AthenaCassieCommandReceiver::LatchInitialPosition(
...
void AthenaCassieCommandReceiver::DoCalcNextUpdateTime(
    const systems::Context<double>& context,
...
void AthenaCassieCommandReceiver::CalcDefaultedCommand(
    const systems::Context<double>& context, lcmt_cassie_command* result) const {
  // Copy the input value into our tentative result.
  *result = message_input_->Eval<lcmt_cassie_command>(context);

  // If we haven't received a message yet, then fall back to the default
  // position.
  if (lcm::AreLcmMessagesEqual(*result, lcmt_cassie_command{})) {
...
void AthenaCassieCommandReceiver::CalcPositionOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_cassie_command>(context);
  if (message.num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "AthenaCassieCommandReceiver expected num_joints = {}, but received {}",
...
void AthenaCassieCommandReceiver::CalcTorqueOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_cassie_command>(context);
  ...
  if (message.num_torques != num_joints_) {
    throw std::runtime_error(fmt::format(
        "AthenaCassieCommandReceiver expected num_torques = {}, but received {}",
