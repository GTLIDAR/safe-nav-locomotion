#include "drake/examples/athena_cassie/athena_cassie_common.h"
...
void SetTorqueControlledAthenaCassieGains(Eigen::VectorXd* stiffness,
  ...
  stiffness->resize(75);
  *stiffness << 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000,
...
void ApplyJointVelocityLimits(const MatrixX<double>& keyframes,
  ...
  DRAKE_ASSERT(velocities.rows() == kAthenaCassieNumJoints);

  Eigen::VectorXd velocity_ratios(velocities.rows());

  const VectorX<double> athena_cassie_max_joint_velocities =
      get_athena_cassie_max_joint_velocities();
  for (int i = 0; i < velocities.rows(); i++) {
    const double max_plan_velocity = velocities.row(i).maxCoeff();
    // Maybe don't try max velocity at first...
    velocity_ratios(i) =
        max_plan_velocity / (athena_cassie_max_joint_velocities[i] * 0.9);
  }
