...
/// Creates and outputs lcmt_turtle_status messages.
///
...
/// systems::lcm::LcmPublisherSystem::Make<lcmt_turtle_status>().
///
...
///     comments in `lcmt_turtle_status.lcm`); when absent, the output message
///     will use zeros.
...
///
/// This system has one abstract-valued output port of type lcmt_turtle_status.
///
...
///   @output_port{lcmt_turtle_status}
/// }
/// @see `lcmt_turtle_status.lcm` for additional documentation.
class TurtleStatusSender : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(TurtleStatusSender)

  explicit TurtleStatusSender(int num_joints = kTurtleNumJoints);
 ...
 private:
  void CalcOutput(const systems::Context<double>&, lcmt_turtle_status*) const;
