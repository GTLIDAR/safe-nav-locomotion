...
TurtleCommandReceiver::TurtleCommandReceiver(int num_joints)
    : num_joints_(num_joints) {
  DRAKE_THROW_UNLESS(num_joints > 0);

  message_input_ = &DeclareAbstractInputPort(
      "lcmt_turtle_command", Value<lcmt_turtle_command>());
  ...
  position_measured_or_param_ = &DeclareCacheEntry(
      ...
      &TurtleCommandReceiver::CalcPositionMeasuredOrParam,
  ...
  defaulted_command_ = &DeclareCacheEntry(
      "defaulted_command", &TurtleCommandReceiver::CalcDefaultedCommand,
  ...
  commanded_position_output_ = &DeclareVectorOutputPort(
      ...
      &TurtleCommandReceiver::CalcPositionOutput,
      {defaulted_command_->ticket()});

  commanded_torque_output_ = &DeclareVectorOutputPort(
      "torque", BasicVector<double>(num_joints),
      &TurtleCommandReceiver::CalcTorqueOutput,
      {defaulted_command_->ticket()});
}

void TurtleCommandReceiver::set_initial_position(
...
void TurtleCommandReceiver::CalcPositionMeasuredOrParam(
...
void TurtleCommandReceiver::LatchInitialPosition(
...
void TurtleCommandReceiver::DoCalcNextUpdateTime(
    const systems::Context<double>& context,
...
void TurtleCommandReceiver::CalcDefaultedCommand(
    const systems::Context<double>& context, lcmt_turtle_command* result) const {
  // Copy the input value into our tentative result.
  *result = message_input_->Eval<lcmt_turtle_command>(context);

  // If we haven't received a message yet, then fall back to the default
  // position.
  if (lcm::AreLcmMessagesEqual(*result, lcmt_turtle_command{})) {
...
void TurtleCommandReceiver::CalcPositionOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_turtle_command>(context);
  if (message.num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "TurtleCommandReceiver expected num_joints = {}, but received {}",
...
void TurtleCommandReceiver::CalcTorqueOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_turtle_command>(context);
  ...
  if (message.num_torques != num_joints_) {
    throw std::runtime_error(fmt::format(
        "TurtleCommandReceiver expected num_torques = {}, but received {}",
