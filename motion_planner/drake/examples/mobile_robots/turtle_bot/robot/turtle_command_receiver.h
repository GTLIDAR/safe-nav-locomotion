...
/// Handles lcmt_turtle_command message from a LcmSubscriberSystem.
///
...
/// LcmSubscriberSystem::Make<drake::lcmt_turtle_command>().
///
/// It has one required input port, "lcmt_turtle_command".
///
...
///
/// @system{turtleCommandReceiver,
///   @input_port{lcmt_turtle_command}
///   @input_port{position_measured (optional)},
///   @output_port{position}
///   @output_port{torque}
/// }
///
/// @par Output prior to receiving a valid lcmt_turtle_command message:
...
class TurtleCommandReceiver : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(TurtleCommandReceiver)

  explicit TurtleCommandReceiver(int num_joints = kTurtleNumJoints);

  /// Sets the initial commanded position of the controlled turtle prior to any
 ...
 private:
  ...
  void CalcDefaultedCommand(
      const systems::Context<double>&, lcmt_turtle_command*) const;
