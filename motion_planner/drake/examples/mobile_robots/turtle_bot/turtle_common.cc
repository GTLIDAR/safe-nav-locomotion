#include "drake/examples/mobile_robots/turtle_bot/turtle_common.h"
...
void ApplyJointVelocityLimits(const MatrixX<double>& keyframes,
  ...
  DRAKE_ASSERT(velocities.rows() == kTurtleNumJoints);

  Eigen::VectorXd velocity_ratios(velocities.rows());

  const VectorX<double> turtle_max_joint_velocities =
      get_turtle_max_joint_velocities();
  for (int i = 0; i < velocities.rows(); i++) {
    const double max_plan_velocity = velocities.row(i).maxCoeff();
    // Maybe don't try max velocity at first...
    velocity_ratios(i) =
        max_plan_velocity / (turtle_max_joint_velocities[i] * 0.9);
  }
