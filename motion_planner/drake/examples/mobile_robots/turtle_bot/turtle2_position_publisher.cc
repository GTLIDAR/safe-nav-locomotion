...
const char* const kLcmStatusChannel = "TURTLE_2_STATUS";
const char* const kLcmTimeChannel = "TURTLE_2_TIME";
const char* const kLcmObjectTargetChannel = "OBJECT_TARGET_2";

using drake::math::RigidTransform;
using Eigen::Vector3d;

class Turtle2PositionPublisher {
 public:

  explicit Turtle2PositionPublisher(const multibody::MultibodyPlant<double>& plant)
      : plant_(plant) {
    lcm_.subscribe(kLcmTimeChannel,
                    &Turtle2PositionPublisher::HandleRobotTime, this);
    lcm_.subscribe(kLcmObjectTargetChannel,
                    &Turtle2PositionPublisher::HandleTarget, this);
  }
  ...
  void Run(Json::Value root, std::vector<Eigen::Matrix<double, 3, 1>> location_list, 
    ...
    while (true) {
      ...
      if(!already_moving){
        ...
        already_moving = true;
        drake::log()->info("turtle 2 started moving");
      }
...
int do_main() {
  ...
  Turtle2PositionPublisher position_publisher(plant);
