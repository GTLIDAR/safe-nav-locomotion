...
QuadrotorCommandSender::QuadrotorCommandSender(int num_joints)
  ...
  this->DeclareAbstractOutputPort(
      "lcmt_quadrotor_command", &QuadrotorCommandSender::CalcOutput);
}

using InPort = systems::InputPort<double>;
const InPort& QuadrotorCommandSender::get_position_input_port() const {
  return LeafSystem<double>::get_input_port(0);
}
const InPort& QuadrotorCommandSender::get_torque_input_port() const {
  return LeafSystem<double>::get_input_port(1);
}
const systems::OutputPort<double>& QuadrotorCommandSender::get_output_port() const {
  return LeafSystem<double>::get_output_port(0);
}

void QuadrotorCommandSender::CalcOutput(
    const systems::Context<double>& context, lcmt_quadrotor_command* output) const {
  ...
  lcmt_quadrotor_command& command = *output;
