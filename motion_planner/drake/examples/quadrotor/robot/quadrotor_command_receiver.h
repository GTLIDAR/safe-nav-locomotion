...
/// Handles lcmt_quadrotor_command message from a LcmSubscriberSystem.
///
...
/// LcmSubscriberSystem::Make<drake::lcmt_quadrotor_command>().
///
/// It has one required input port, "lcmt_quadrotor_command".
///
...
///
/// @system{quadrotorCommandReceiver,
///   @input_port{lcmt_quadrotor_command}
///   @input_port{position_measured (optional)},
///   @output_port{position}
///   @output_port{torque}
/// }
///
/// @par Output prior to receiving a valid lcmt_quadrotor_command message:
...
class QuadrotorCommandReceiver : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(QuadrotorCommandReceiver)

  explicit QuadrotorCommandReceiver(int num_joints = kQuadrotorNumJoints);

  /// Sets the initial commanded position of the controlled quadrotor prior to any
 ...
 private:
  ...
  void CalcDefaultedCommand(
      const systems::Context<double>&, lcmt_quadrotor_command*) const;
