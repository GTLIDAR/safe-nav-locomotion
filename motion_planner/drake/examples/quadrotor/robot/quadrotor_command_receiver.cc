...
QuadrotorCommandReceiver::QuadrotorCommandReceiver(int num_joints)
    : num_joints_(num_joints) {
  DRAKE_THROW_UNLESS(num_joints > 0);

  message_input_ = &DeclareAbstractInputPort(
      "lcmt_quadrotor_command", Value<lcmt_quadrotor_command>());
  ...
  position_measured_or_param_ = &DeclareCacheEntry(
      ...
      &QuadrotorCommandReceiver::CalcPositionMeasuredOrParam,
  ...
  defaulted_command_ = &DeclareCacheEntry(
      "defaulted_command", &QuadrotorCommandReceiver::CalcDefaultedCommand,
  ...
  commanded_position_output_ = &DeclareVectorOutputPort(
      ...
      &QuadrotorCommandReceiver::CalcPositionOutput,
      {defaulted_command_->ticket()});

  commanded_torque_output_ = &DeclareVectorOutputPort(
      "torque", BasicVector<double>(num_joints),
      &QuadrotorCommandReceiver::CalcTorqueOutput,
      {defaulted_command_->ticket()});
}

void QuadrotorCommandReceiver::set_initial_position(
...
void QuadrotorCommandReceiver::CalcPositionMeasuredOrParam(
...
void QuadrotorCommandReceiver::LatchInitialPosition(
...
void QuadrotorCommandReceiver::DoCalcNextUpdateTime(
    const systems::Context<double>& context,
...
void QuadrotorCommandReceiver::CalcDefaultedCommand(
    const systems::Context<double>& context, lcmt_quadrotor_command* result) const {
  // Copy the input value into our tentative result.
  *result = message_input_->Eval<lcmt_quadrotor_command>(context);

  // If we haven't received a message yet, then fall back to the default
  // position.
  if (lcm::AreLcmMessagesEqual(*result, lcmt_quadrotor_command{})) {
...
void QuadrotorCommandReceiver::CalcPositionOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_quadrotor_command>(context);
  if (message.num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "QuadrotorCommandReceiver expected num_joints = {}, but received {}",
...
void QuadrotorCommandReceiver::CalcTorqueOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_quadrotor_command>(context);
  ...
  if (message.num_torques != num_joints_) {
    throw std::runtime_error(fmt::format(
        "QuadrotorCommandReceiver expected num_torques = {}, but received {}",
