...
/// Creates and outputs lcmt_quadrotor_status messages.
///
...
/// systems::lcm::LcmPublisherSystem::Make<lcmt_quadrotor_status>().
///
...
///     comments in `lcmt_quadrotor_status.lcm`); when absent, the output message
///     will use zeros.
...
///
/// This system has one abstract-valued output port of type lcmt_quadrotor_status.
///
...
///   @output_port{lcmt_quadrotor_status}
/// }
/// @see `lcmt_quadrotor_status.lcm` for additional documentation.
class QuadrotorStatusSender : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(QuadrotorStatusSender)

  explicit QuadrotorStatusSender(int num_joints = kQuadrotorNumJoints);
 ...
 private:
  void CalcOutput(const systems::Context<double>&, lcmt_quadrotor_status*) const;
