...
int do_main() {
  ...
  const std::string full_name =
      FindResourceOrThrow("drake/manipulation/models/cassie_description/urdf/cassie.urdf");
  ...
  drake::log()->info("num pos: {}", plant.num_positions());
  DRAKE_DEMAND(plant.num_velocities() == 20);
  DRAKE_DEMAND(plant.num_positions() == 21);
