...
const char* const kLcmStatusChannel = "OBJECT_STATUS";
const char* const kLcmTimeChannel = "OBJECT_TIME";
...
class ObjectPositionPublisher {
 public:
  ...
  explicit ObjectPositionPublisher(const multibody::MultibodyPlant<double>& plant)
      : plant_(plant) {
    lcm_.subscribe(kLcmTimeChannel,
                    &ObjectPositionPublisher::HandleRobotTime, this);
  }
  ...
  void Run(Json::Value root, std::vector<Eigen::Matrix<double, 3, 1>> location_list) {
    ...
    object_state.num_joints = num_joints_;
    object_state.joint_position_measured.resize(num_joints_, 0.);   
    object_state.joint_velocity_estimated.resize(num_joints_, 0.);
    object_state.joint_position_commanded.resize(num_joints_, 0.);
    object_state.joint_position_ipo.resize(num_joints_, 0.);
    object_state.joint_torque_measured.resize(num_joints_, 0.);
    object_state.joint_torque_commanded.resize(num_joints_, 0.);
    object_state.joint_torque_external.resize(num_joints_, 0.);
  ...
  while (true) {
      ...
      // processed.
      while (0 == lcm_.handleTimeout(10) || object_state.utime == -1 
              || plan_finished_) { }

      // Update status time to simulation time
      object_state.utime = robot_time_.utime;
      ...
      for (int joint = 0; joint < num_joints_; joint++) 
      {
        ...
        {
          object_state.joint_position_measured[joint] = location_list[step_](joint-4, 0);
        }
      }

      // Temporary Hack: 
      object_state.joint_position_measured[4] = location_list[step_](0, 0)+2;
      object_state.joint_position_measured[5] = location_list[step_](1, 0)-6;

      lcm_.publish(kLcmStatusChannel, &object_state);
    }
  }
...
int do_main() {
  ...
  ObjectPositionPublisher position_publisher(plant);
