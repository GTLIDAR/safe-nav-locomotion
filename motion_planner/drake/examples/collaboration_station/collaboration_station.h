...
// #include "drake/examples/collaboration_station/include/json.hpp"
...
/// Determines which sdf is loaded for the IIWA in the CollaborationStation.
enum class IiwaCollisionModel { kNoCollision, kBoxCollision };

enum class AthenaCassieCollisionModel { kNoCollision, kBoxCollision };

/// @defgroup collaboration_station_systems Manipulation Station
/// @{
...
///
/// @tparam_double_only
/// @ingroup collaboration_station_systems
template <typename T>
class CollaborationStation : public systems::Diagram<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(CollaborationStation)
  ...
  ///   command inputs.
  explicit CollaborationStation(double time_step = 0.002);
  ...
  void SetupHomeEnvironmentV1(
      IiwaCollisionModel iiwa_collision_model = IiwaCollisionModel::kNoCollision,
      AthenaCassieCollisionModel athena_cassie_collision_model = AthenaCassieCollisionModel::kNoCollision
  );

  /// Sets the default State for the chosen setup.
  /// @param context A const reference to the CollaborationStation context.
  /// @param state A pointer to the State of the CollaborationStation system.
  ...
  /// Notifies the CollaborationStation that the IIWA robot model instance can
  ...
  /// Notifies the CollaborationStation that the WSG gripper model instance can
  ...
  void RegisterAthenaCassieControllerModel(
      const std::string& model_path,
  ...
  int num_athena_cassie_joints() const;
  ...
  /// Convenience method for getting all of the joint angles of the Cassie.
  /// This does not include the gripper.
  VectorX<T> GetAthenaCassiePosition(const systems::Context<T>& station_context) const;

  /// Convenience method for setting all of the joint angles of the Cassie.
  /// Also sets the position history in the velocity command generator.
  /// @p q must have size num_iiwa_joints().
  /// @pre `state` must be the systems::State<T> object contained in
  /// `station_context`.
  void SetAthenaCassiePosition(const systems::Context<T>& station_context,
                       systems::State<T>* state,
                       const Eigen::Ref<const VectorX<T>>& q) const;

  /// Convenience method for setting all of the joint angles of the cassie.
  /// Also sets the position history in the velocity command generator.
  /// @p q must have size num_cassie_joints().
  void SetAthenaCassiePosition(systems::Context<T>* station_context,
                       const Eigen::Ref<const VectorX<T>>& q) const {
    SetAthenaCassiePosition(*station_context, &station_context->get_mutable_state(), q);
  }

  /// Convenience method for getting all of the joint velocities of the Kuka
  // IIWA.  This does not include the gripper.
  VectorX<T> GetAthenaCassieVelocity(const systems::Context<T>& station_context) const;

  /// Convenience method for setting all of the joint velocities of the Cassie
  /// @v must have size num_cassie_joints().
  /// @pre `state` must be the systems::State<T> object contained in
  /// `station_context`.
  void SetAthenaCassieVelocity(const systems::Context<T>& station_context,
                       systems::State<T>* state,
                       const Eigen::Ref<const VectorX<T>>& v) const;

  /// Convenience method for setting all of the joint velocities of the Cassie
  /// @v must have size num_cassie_joints().
  void SetAthenaCassieVelocity(systems::Context<T>* station_context,
                       const Eigen::Ref<const VectorX<T>>& v) const {
    SetAthenaCassieVelocity(*station_context, &station_context->get_mutable_state(), v);
  }
 ...
 private:
  ...
  void MakeAthenaCassieControllerModel();

  void AddDefaultIiwa(const IiwaCollisionModel collision_model);
  void AddDefaultWsg();
  void AddDefaultAthenaCassie(const AthenaCassieCollisionModel athena_cassie_collision_model);
  ...
  ModelInformation athena_cassie_model_;
  ...
  // These are kp and kd gains for iiwa and wsg controllers.
  VectorX<double> athena_cassie_kp_;
  VectorX<double> athena_cassie_kd_;
  VectorX<double> athena_cassie_ki_;

  VectorX<double> robots_kp_;
  VectorX<double> robots_kd_;
  VectorX<double> robots_ki_;
