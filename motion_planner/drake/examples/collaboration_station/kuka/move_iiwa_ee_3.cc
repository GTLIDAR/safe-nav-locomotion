...
#include "drake/examples/collaboration_station/kuka/high_level_planner_3.h"
...
const char* const kLcmIiwaStatusChannel = "IIWA_STATUS_3";
const char* const kLcmWsgStatusChannel = "WSG_STATUS_3";
const char* const kLcmMultiObjectStatusChannel = "MULTI_OBJECT_STATUS_2";
const char* const kLcmObjectTargetChannel = "OBJECT_TARGET_3";
const char* const kLcmPlanChannel = "COMMITTED_ROBOT_PLAN_3";
const char* const kLcmWsgPlanChannel = "WSG_PLAN_3";
...
class MoveDemoRunner {
 public:
  ...
  void Run() {
    ...
    Vector6<double> iiwa_3_pose;
    iiwa_3_pose << root["iiwa_3_pose"][0].asDouble(),root["iiwa_3_pose"][1].asDouble(),
                   root["iiwa_3_pose"][2].asDouble(),root["iiwa_3_pose"][3].asDouble(),
                   root["iiwa_3_pose"][4].asDouble(),root["iiwa_3_pose"][5].asDouble();

    num_of_objects_ = root["num_of_objects_to_use_2"].asInt();


    // Second parameter: set 0 for set kuka to cabinet, 1 for kuka to box
    HighLevelPlanner planner(iiwa_3_pose,
    ...
    MoveIkDemoBase demo(
        !FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(root["robot_iiwa_3_path"].asString()),
        !FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(root["robot_wsg_3_path"].asString()),
...
drake::log()->info("initialized: {}",num_of_objects_);
    ...
    if(num_of_objects_ != 1){
      ...
      object_correct_order.push_back(0);
    }

      drake::log()->info("outside while");
    ...
    while(true){
      ...
      if(task.compare("new_object") == 0){
        ...
        const Eigen::Quaternion<double> obj_quat(
          root["multi_object_position_2"][object_list[object_num]][0].asDouble(),
          root["multi_object_position_2"][object_list[object_num]][1].asDouble(),
          root["multi_object_position_2"][object_list[object_num]][2].asDouble(),
          root["multi_object_position_2"][object_list[object_num]][3].asDouble()
        );
        const math::RollPitchYaw<double> rpy(obj_quat);
        ...
        // if(chosen_plan.compare("z") == 0){
        //   object_target.roll_angle += 3.14; //Wsg is upside down
        // }
        // else if(chosen_plan.compare("y") == 0){
        //   object_target.pitch_angle = 0;
        // }
        // else if(chosen_plan.compare("x") == 0){
        //   object_target.yaw_angle = 0;
        // }
      ...
      if(demo.IsFinished()){
        ...
        else if(planner.HasMoreObjects()){
          planner.NextObject();
    ...
    drake::log()->info("move iiwa 3 has finished completely");
...
int main(int argc, char* argv[]) {
  ...
  runner.Run();
  drake::log()->info("move_iiwa_ee_3.cc has finished");
  return 0;
}
