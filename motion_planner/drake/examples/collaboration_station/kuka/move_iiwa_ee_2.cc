...
#include "drake/examples/collaboration_station/kuka/high_level_planner_2.h"
...
const char* const kLcmIiwaStatusChannel = "IIWA_STATUS_2";
const char* const kLcmWsgStatusChannel = "WSG_STATUS_2";
...
const char* const kLcmWsgPlanChannel = "WSG_PLAN_2";
...
class MoveDemoRunner {
 public:
  ...
  void Run() {
    ...
    iiwa_2_pose << root["iiwa_2_pose"][0].asDouble(),root["iiwa_2_pose"][1].asDouble(),
                   root["iiwa_2_pose"][2].asDouble(),root["iiwa_2_pose"][3].asDouble(),
                   root["iiwa_2_pose"][4].asDouble(),root["iiwa_2_pose"][5].asDouble();

    num_of_objects_ = root["num_of_objects_to_use_1"].asInt();

    HighLevelPlanner planner(iiwa_2_pose,
      "drake/examples/collaboration_station/pose_list_2.json",
      1,
      num_of_objects_
    );

    MoveIkDemoBase demo(
        !FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(root["robot_iiwa_2_path"].asString()),
        !FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(root["robot_wsg_2_path"].asString()),
...
drake::log()->info("correct num size: {} ", multi_object_states_.size());
    ...
    //Makes sure to send a signal to athena-cassie
    int rand_counter = 0;
    ...
    while(true){
      lcm_.publish(kLcmObjectTargetChannel, &object_target);
      rand_counter++;
      if(rand_counter > 10000){
        break;
      }
    }
    drake::log()->info("move iiwa has finished completely");
  }
...
int main(int argc, char* argv[]) {
  ...
  runner.Run();
  drake::log()->info("move_iiwa_ee_2.cc has finished");
  return 0;
}
