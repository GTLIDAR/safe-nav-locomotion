...
Vector6<double> HighLevelPlanner::NewObject(){
  object_name_ = object_list_["list2"][object_num_].asString();
...
bool HighLevelPlanner::HasMoreObjects(){
...
void HighLevelPlanner::MoveToNewPlan(){
  task_num_++;
  current_task_ = pose_list_["tasks"][task_num_].asString();
}
...
std::vector<std::string> HighLevelPlanner::GetObjectList(){
  ...
  for(int i = 0; i < t; i++){
    temp.push_back(object_list_["list2"][i].asString());
  }
  return temp;
}

double HighLevelPlanner::Print(){
  return object_list_[object_list_["list2"][object_num_].asString()]["size"][1].asDouble()/2; 
}
