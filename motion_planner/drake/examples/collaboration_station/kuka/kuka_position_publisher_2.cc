...
const char* const kLcmTargetChannel = "OBJECT_TARGET_2";
const char* const kLcmSchunkChannel = "WSG_STATUS_2";
const char* const kLcmIiwaChannel = "IIWA_STATUS_2";
const char* const kLcmTimeChannel = "IIWA_TIME_2";
const char* const kLcmBoxChannel = "ATHENA_CASSIE_STATUS";
const char* const kLcmPlanChannel = "COMMITTED_ROBOT_PLAN_2";
const char* const kLcmStopChannel = "STOP_2";
...
class RobotPlanRunner {
 public:
  ...
  void Run(Json::Value root, Json::Value object_list) {
    ...
    full_path = root["path_to_drake"].asString() + root["log_iiwa_2_read"].asString();
    std::vector<Eigen::Matrix<double, 7, 1>> iiwa_2_list;
    read_data<7>(iiwa_2_list, full_path);

    full_path = root["path_to_drake"].asString() + root["log_wsg_2_read"].asString();
    std::vector<Eigen::Matrix<double, 2, 1>> wsg_2_list;
    read_data<2>(wsg_2_list, full_path);

    full_path = root["path_to_drake"].asString() + root["log_multi_object_2_read"].asString();
    std::vector<Eigen::Matrix<double, 21, 1>> multi_object_2_list;
    read_data<21>(multi_object_2_list, full_path);

    full_path = root["path_to_drake"].asString() + root["log_step_2_read"].asString();
    std::vector<Eigen::Matrix<double, 1, 1>> step_2;
    read_data<1>(step_2, full_path);
    const int max_steps = step_2[0](0,0);
    ...
    for(int j = 0; j < num_of_objects; j++){
      for(int i = 0; i < 7; i++)  {
        std::string tmp2 = root["multi_object_order_1"][j].asString();
        const double obj_pos = root["multi_object_position_1"][tmp2][i].asDouble();
    ...
    while (true) {
      ...
      cur_time_us = iiwa_time_.utime;
        ...
        if(step >= max_steps){
          ...
          for (int joint = 0; joint < 7; joint++) {
            iiwa_status.joint_position_measured[joint] = iiwa_2_list[step](joint,0);
          }
          wsg_status.actual_position_mm = wsg_2_list[step](0,0) - wsg_2_list[step](1,0);
          for(int joint = 0; joint < num_obj_joints; joint++){
            object_state_.joint_position_measured[joint] = multi_object_2_list[step](joint,0);
          }
      ...
      std::string output_file_name = root["path_to_drake"].asString() + root["log_iiwa_2"].asString();
      ...
      output_file_name = root["path_to_drake"].asString() + root["log_wsg_2"].asString();
      ...
      output_file_name = root["path_to_drake"].asString() + root["log_multi_object_2"].asString();
      ...
      output_file_name = root["path_to_drake"].asString() + root["log_step_2"].asString();
