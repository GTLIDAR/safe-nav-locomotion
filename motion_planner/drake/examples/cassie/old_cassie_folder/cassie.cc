...
namespace cassie{
  ...
  Cassie::Cassie()
  {  
    DeclareDiscreteState(23); 
    DeclareVectorOutputPort("floating_base_state", drake::systems::BasicVector<double>(23),
                                                      ...
                                                      const Eigen::Matrix<double, 1, 1>& heading) const
  {
    ...
    res(13, 0) = phi_left - res(11, 0) - 5.11 * PI / 180; // knee motor joint angle
    res(15, 0) = 0; // knee joint angle, 0 cuz no deformation
    res(17, 0) = 13 * PI / 180 - res(13, 0); // ankle joint angle
    res(19, 0) = 0; // ankle spring joint angle, 0 cuz no deformation

    // toe, always parallel to ground
    res(20,0) = - 0.88 - res(17, 0) - res(13, 0) - res(11, 0); // toe joint angle, 0.88 got by trial (when other joint angles are 0, angle the toe needs to rotate to be parallel to ground)
    ...
    res(12, 0) = PI / 2-13 * PI / 180 - (std::atan2(q_toe_right, p_toe_right) - gamma_right - delta_right);
    double phi_right = PI / 2 - std::atan2(q_toe_right, p_toe_right) - std::acos((l_right*l_right + a*a - d*d) / 2 / l_right / a);
    res(14, 0) = phi_right - res(12, 0) - 5.11 * PI / 180;
    res(16, 0) = 0;
    res(18, 0) = 13 * PI / 180 - res(14, 0);
    res(21, 0) = 0;


    res(22,0) = - 0.88 - res(18, 0) - res(14, 0) - res(12, 0);
