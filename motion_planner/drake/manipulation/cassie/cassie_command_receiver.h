...
#include "drake/manipulation/cassie/cassie_constants.h"
...
/// Handles lcmt_cassie_command message from a LcmSubscriberSystem.
///
...
/// LcmSubscriberSystem::Make<drake::lcmt_cassie_command>().
///
/// It has one required input port, "lcmt_cassie_command".
///
...
///
/// @system{CassieCommandReceiver,
///   @input_port{lcmt_cassie_command}
///   @input_port{position_measured (optional)},
///   @output_port{position}
///   @output_port{torque}
/// }
///
/// @par Output prior to receiving a valid lcmt_cassie_command message:
...
class CassieCommandReceiver : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(CassieCommandReceiver)

  explicit CassieCommandReceiver(int num_joints = kCassieNumJoints);

  /// Sets the initial commanded position of the controlled cassie prior to any
 ...
 private:
  ...
  void CalcDefaultedCommand(
      const systems::Context<double>&, lcmt_cassie_command*) const;
