...
/// Creates and outputs lcmt_cassie_status messages.
///
...
///     comments in `lcmt_cassie_status.lcm`); when absent, the output message
///     will use zeros.
...
///
/// This system has one abstract-valued output port of type lcmt_cassie_status.
///
...
///   @output_port{lcmt_cassie_status}
/// }
/// @see `lcmt_cassie_status.lcm` for additional documentation.
class CassieStatusSender : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(CassieStatusSender)

  explicit CassieStatusSender(int num_joints = kCassieNumJoints);
 ...
 private:
  void CalcOutput(const systems::Context<double>&, lcmt_cassie_status*) const;
