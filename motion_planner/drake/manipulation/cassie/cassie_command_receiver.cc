...
CassieCommandReceiver::CassieCommandReceiver(int num_joints)
  ...
  position_measured_or_param_ = &DeclareCacheEntry(
      ...
      &CassieCommandReceiver::CalcPositionMeasuredOrParam,
  ...
  defaulted_command_ = &DeclareCacheEntry(
      "defaulted_command", &CassieCommandReceiver::CalcDefaultedCommand,
  ...
  commanded_position_output_ = &DeclareVectorOutputPort(
      ...
      &CassieCommandReceiver::CalcPositionOutput,
      {defaulted_command_->ticket()});

  commanded_torque_output_ = &DeclareVectorOutputPort(
      "torque", BasicVector<double>(num_joints),
      &CassieCommandReceiver::CalcTorqueOutput,
      {defaulted_command_->ticket()});
}

void CassieCommandReceiver::set_initial_position(
...
void CassieCommandReceiver::CalcPositionMeasuredOrParam(
...
void CassieCommandReceiver::LatchInitialPosition(
...
void CassieCommandReceiver::DoCalcNextUpdateTime(
    const systems::Context<double>& context,
...
void CassieCommandReceiver::CalcDefaultedCommand(
...
void CassieCommandReceiver::CalcPositionOutput(
  ...
  if (message.num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "CassieCommandReceiver expected num_joints = {}, but received {}",
...
void CassieCommandReceiver::CalcTorqueOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = defaulted_command_->Eval<lcmt_cassie_command>(context);
  ...
  if (message.num_torques != num_joints_) {
    throw std::runtime_error(fmt::format(
        "CassieCommandReceiver expected num_torques = {}, but received {}",
