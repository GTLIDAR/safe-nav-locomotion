#include "drake/manipulation/planner/constraint_relaxing_ik_kuka.h"
...
ConstraintRelaxingIkKuka::ConstraintRelaxingIkKuka(
  ...
  // plant_ = *builder.AddSystem<drake::multibody::MultibodyPlant>(0);
  ...
  // link.
  if (plant.GetBodiesWeldedTo(plant.world_body()).size() <= 1) {
    const std::vector<multibody::BodyIndex> bodies =
        plant.GetBodyIndices(model_instance);
    plant.WeldFrames(plant.world_frame(),
                      plant.get_body(bodies[0]).body_frame());
  }
  plant.Finalize();
...
bool ConstraintRelaxingIkKuka::PlanSequentialTrajectory(
...
bool ConstraintRelaxingIkKuka::SolveIk(
  ...
  ik.AddMinimumDistanceConstraint(0.1);
