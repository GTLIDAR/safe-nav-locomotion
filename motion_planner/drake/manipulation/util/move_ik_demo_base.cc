...
MoveIkDemoBase::MoveIkDemoBase(std::string robot_description,
                               std::string wsg_description,
                               std::string base_link,
                               std::string ik_link,
                               int print_interval)
    : robot_description_(std::move(robot_description)),
      wsg_description_(std::move(wsg_description)),
      ik_link_(std::move(ik_link)),
      print_interval_(print_interval),
      plant_(0.0),
      constraint_relaxing_ik(robot_description_, ik_link_) {


  if(wsg_description_.empty())
  {
  ...
  {
    drake::log()->info("plant has wsg");

    iiwa_instance_ =
      multibody::Parser(&plant_).AddModelFromFile(robot_description_);
    multibody::ModelInstanceIndex wsg_instance = 
      multibody::Parser(&plant_).AddModelFromFile(wsg_description_);

    plant_.WeldFrames(plant_.world_frame(),
                      plant_.GetBodyByName(base_link).body_frame());
    const multibody::Frame<double>& link7 =
        plant_.GetFrameByName("iiwa_link_7", iiwa_instance_);
    const math::RigidTransformd X_7G(
        math::RollPitchYaw<double>(M_PI_2, 0, M_PI_2), Eigen::Vector3d(0, 0, 0.114));
    plant_.WeldFrames(link7, plant_.GetFrameByName(
...
std::optional<robotlocomotion::robot_plan_t> MoveIkDemoBase::Plan(
  ...
  const bool result =
      constraint_relaxing_ik.PlanSequentialTrajectory(
          waypoints, plant_.GetPositions(*context_, iiwa_instance_), &q_sol);
  // drake::log()->info("IK result: {}", result);

  // This vector contains iiwa and wsg plans
  std::vector<Eigen::VectorXd> current_q_all(q_sol.size());
  ...
  if (result) {
    ...
    for(int i = 0; i < temp; i++){
      current_q_all[i].resize(9);
      Eigen::VectorXd w(9);
      w << q_sol[i], wsg_position/2, -wsg_position/2;
      current_q_all[i] << w; 
    }

    //Since there's only 2 values, the current is always teh first one
    current_goal_ = current_q_all[current_q_all.size()-1]; 

    std::vector<int> info{1, 1};
    robotlocomotion::robot_plan_t plan = EncodeKeyFrames(
        joint_names_, times, info, current_q_all);
    return plan;
  }
  return std::nullopt;
}

bool MoveIkDemoBase::IsFinished(){
  DRAKE_DEMAND(current_goal_.size() > 0);
  DRAKE_DEMAND(current_q_.size() > 0);

  for(int i = 0; i < joint_velocity_limits_.size(); i++)
  {
    if(!(current_goal_[i] < 0.01+current_q_[i] && 
        current_goal_[i] > -0.01+current_q_[i]))
    {
      return false;
    }
  }
